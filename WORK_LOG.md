...
- 규칙 7·8의 "공감하는 톤으로" → "환자가 부담을 느끼지 않도록 담담하고 편안하게"로 교체 (민감 주제 배려라는 원래 의도를 명확히, "공감=반응성 멘트" 오해 제거).
- [공통 규칙]에 평가성 도입부 금지 추가 — "~원하신다니 다행입니다", "~하려 하신다니 좋습니다", "잘 결정하셨습니다" 등 환자 의향을 좋다/잘했다고 반응하는 표현 금지. 인사·평가 없이 바로 안내 시작. (규칙 6 등 정해진 답변 형식은 예외)

**검증**: `npm run build` 통과. 라이브 응답 테스트는 Anthropic API 과부하(529)로 보류 — API 회복 후 재확인 필요.
