...
### 마크다운 룰 회귀 픽스 — 줄바꿈 가독성 복구

**계기**: 사용자 production 테스트 — 진료시간 답변이 줄바꿈 없이 한 줄로 길게 나와 가독성 저하. 전에는 평일/토요일/휴진을 줄 나눠 안내했음.

**원인**: 같은 날 추가한 마크다운 금지 룰에 "- 목록 금지"가 있어 모델(haiku)이 줄바꿈 자체를 과하게 회피. 줄바꿈(`\n`)은 마크다운이 아니고 `MessageText` 가 `<br>` 로 렌더링하는데, 모델이 구분 못 하고 한 문단에 몰아씀.

**한 것** (`lib/prompts/safety/common.js`)
- [공통 규칙]의 마크다운 룰을 둘로 분리: (1) 마크다운 **기호**(별표/대괄호링크/#/글머리표) 금지 — 평문만. (2) **가독성 줄바꿈은 권장** — 나열형 정보(진료시간·비용 등)는 줄바꿈으로 나눠 안내, 줄바꿈은 마크다운 아니므로 그대로 표시됨.
- 검증: dev 서버에서 진료시간 질문 반복 — 평일/토요일/휴진 줄바꿈 복구(줄바꿈 4~6), 마크다운 0건 유지. `npm run build` 통과.
