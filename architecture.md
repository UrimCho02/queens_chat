...
| `lib/prompts/safety/` | **의료법 안전장치.** AI가 진단·위험한 답을 못 하게 막는 규칙 |
...
| 영역 | 잘못 건드리면 |
|---|---|
| `lib/prompts/safety/` | AI가 의학적 진단을 하거나 위험한 답변 → **의료법 위반·법적 책임** |
...
3번의 "설명서"가 핵심입니다. 안전 규칙(`safety/`)이 항상 병원 정보·FAQ보다 **먼저**
...
3. **`lib/prompts/safety/`** — 의료법 안전장치. 사고 시 법적 책임.
...
5. **`lib/prompts/safety/` + `buildPrompt.js`** — AI 응답의 핵심.
...
핵심 원칙: **의료법 가드레일은 코드에 고정**(`lib/prompts/safety/`, 어드민 편집 불가),
...
`safety/`는 의료법 가드레일을 담으며 **어드민 편집 불가**. `safetyRules({ specialty, clinicName, bookingUrl, tone })`이 진입점. `common.js`의 진료과 무관 룰(응급/증상호소/예약/회복가이드 등)과 `{specialty}.js`(`obgyn`/`internal`/`pediatric`)의 진료과 특화 룰(검사·질병·민감 주제)을 합성. `clinics.specialty` 컬럼이 모듈 선택의 키. 모델은 응답에 `STAFF_REQUIRED` / `CATEGORY:<분류>` 태그를 붙이고 route가 파싱한다. 모델은 `claude-haiku-4-5-20251001`.
...
- **진료과 확장** — 현재 `safety/`는 산부인과·내과·소아청소년과 3종 지원. 정형외과·피부과 등 새 진료과는 `safety/{새진료과}.js` 추가 + `clinics.specialty` CHECK 갱신 + `safety/index.js` 모듈 맵 추가.
