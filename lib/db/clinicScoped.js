...
export function clinicScoped(service, table, clinicId) {
  // Fail-fast: clinicId 누락 시 즉시 throw.
  // PostgREST 의 .eq("clinic_id", undefined) 는 필터를 누락시켜 다른 병원
  // 데이터가 누출되거나, INSERT 시 clinic_id NULL 고아 행이 생성됨. RLS 가
  // service_role 을 우회하므로 코드 단에서 막아야 함.
  if (!clinicId) {
    throw new Error(
      "clinicScoped: clinicId is required and cannot be null/undefined."
    );
  }
